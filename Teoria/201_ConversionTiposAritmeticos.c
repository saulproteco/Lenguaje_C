 ...
 * veces se hace de forma implicita al realizar operaciones aritméticas ó   *
 * pasar argumentos a funciones. Las conversiones implicitas se realizan    *
 ...
 * cons signo se preserva el sin signo ya que puede representar más valores *
 * positivos y al operar enteros y números de punto flotante la expresión   *
 * toma el tipo double. Dichas conversiones implicitas también se hacen     *
 * al asignar valores o al pasar argumentos a funciones que tengan          *
 * definido un prototipo en la misma unidad de traducción.                  */

        char a = 10;

        int b = a + 10; // a se convierte implicitamente a entero antes de operar

        double c = b / 6;   // La expresión no se convierte pues los dos
                            // operandos son enteros, pero el resultado si
                            // se convierte implicitamente cuando se asigna.

        func(c);    // El double se convierte implicitamente a _Complex
 ...
 * función que muchas veces es lo mismo) o con argumentos variables.        */
        ...
        c = (double) b / 6; // Se convierte b explicitamente a double
                            // y luego para hacer la operación de
                            // división el compilador convierte el 6
                            // de manera implicita a double para
                            // poder operar.

        a = (char)c + 2;    // Se hace la conversión a char antes de operar.

/* No se puede hacer conversión ni implicita ni explicita entre tipos       *
 ...
 * a menos que la conversión se haga desde o hacia un apuntador a void      */
        ...
        int  * ptrint  = &b;
        char * ptrchar = (char *) ptrint;   // Require conversión explicita
